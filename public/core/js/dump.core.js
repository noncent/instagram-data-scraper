...
var Config = {
  ...
  sessionError: 'No valid session found, Please set a valid session to search @account else search will continue with #hashtag only.',
...
$(document).ajaxError(function(event, jqxhr, settings, thrownError) {
  errStr = '';
  errStr += '<div class="console.info console.info-danger console.info-dismissable">';
  errStr += '  <a href="#" class="close" data-dismiss="console.info" aria-label="close">&times;</a>';
  errStr += '  <strong>OOPS error occured: </strong> Something went wrong or info not available.';
  errStr += '</div>';
  ...
  return $.post($this.sendRequestUrl, send_payload, function(data, textStatus, xhr) {
    ...
    if (send_payload.request_action === 'pull_account') {
      ...
      if ($this.isSearchingForNext === true && $this.infoFullMin === FILTER_FULL) {
        // next ajax call
        console.info('Full request access V');
        _send_payload = {
          // send user name with loop next request
          keyword: $this.info.user_id,
          // request filter
      ...
      $.each(tagListArray, function(i, val) {
        ...
        if (tagListArray[i].indexOf('@') === 0) {
          ...
          if (Cookies.get(instaCookieName)) {
            ...
            console.info(Config.getItem('sessionError'));
          }
          elIndex++;
        }
      });
